...
export default function MatchDetails() {
  ...
  const selectedMeta = useMemo(
    () => comparisonStats.find((s) => s.key === selectedStat) ?? comparisonStats[0],
    [selectedStat]
  );
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white shadow-sm rounded-xl p-4 border">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setShowOverallPanel((v) => !v)}>{showOverallPanel ? "Minimizar" : "Maximizar"}</Button>
        </div>
        ...
        {showOverallPanel && (
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
            {orderedClubs.slice(0, 2).map((c) => {
              ...
              // playoffs do clube
              const playoffs = (playoffsByClub.get(c.clubId) ?? []).slice(0, 20); // limite visual
                    ...
                    <div className="p-2 rounded border">
                      ...
                      <div className="mt-1 flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-col items-center min-w-[48px]">
                          {repUrl && (
                            <img src={repUrl} alt={`Reputação ${repKey ?? ""}`} className="w-10 h-10 object-contain" />
                          )}
                  ...
                  <div key={p.playerId} className="rounded-lg border p-3 bg-white">
                    <div className="flex items-center justify-between">
                      <Link
                        ...
                        to={`/statistics/player/${matchId}/${p.playerId}`}
                      >
                        {p.playerName}
                      </Link>
                      <div className="flex items-center gap-2">
