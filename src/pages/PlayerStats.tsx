...
export default function PlayerStatsPage() {
        ...
        (async () => {
            try {
                ...
                const { data } = await api.get(
                    `/api/matches/${matchId}/players/${playerId}/statistics`,
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const { data: agg } = await api.get(
                    `/api/clubs/${clubId}/players/aggregate?count=${nMatchesPerf}`,
                    { signal: controller.signal }
                );
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const { data } = await api.get(
                    `/api/clubs/${clubId}/players/attributes?count=1`,
                    { signal: controller.signal }
                );
    ...
    const attrComparison = useMemo(() => {
        ...
        if (attrCompareTarget === "avg") {
            const all: PlayerMatchStats[] = teamAttrs
                .map((t) => t.stats)
                .filter((x): x is PlayerMatchStats => !!x);
            keys.forEach((k) => {
        ...
        <div className="p-4 max-w-6xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-3">
                    <a href={shareUrl} className="text-sm text-blue-700 hover:underline">Compartilhar</a>
            ...
            <Card>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                    ...
                    <StatTile label="Precisão de passe" value={fmtPct(passAcc)} />
                            ...
                            <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                <Card>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        ...
                                        <StatCompare label="Assistências" me={perfComparison.me.totalAssists} team={perfComparison.teamAvg.assists} />
                                        <StatCompare label="Chutes" me={perfComparison.me.totalShots} team={perfComparison.teamAvg.shots} />
                                        <StatCompare label="Passe %" me={perfComparison.me.passAccuracyPercent} team={perfComparison.teamAvg.passAcc} pct />
                                        <StatCompare label="Desarmes" me={perfComparison.me.totalTacklesMade} team={perfComparison.teamAvg.tackles} />
                                        <StatCompare label="Desarme %" me={perfComparison.me.tackleSuccessPercent} team={perfComparison.teamAvg.tackAcc} pct />
                                        <StatCompare label="Nota" me={perfComparison.me.avgRating} team={perfComparison.teamAvg.rating} fixed2 />
                                        <StatCompare label="Vitórias %" me={perfComparison.me.winPercent} team={perfComparison.teamAvg.win} pct />
                                    </div>
                                </Card>
                                                ...
                                                {perfComparison.sorted.map((r) => {
                                                    ...
                                                    return (
                                                        <tr key={r.playerId} className={`border-t ${isMe ? "bg-blue-50" : ""}`}>
                                                            <td className="p-2 text-left">{r.playerName}{isMe && " (Você)"}</td>
                                            ...
                                            return (
                                                <div key={String(key)} className="rounded-xl border p-3">
                                                    <div className="flex items-center justify-between text-sm">
                                                        ...
                                                        <span className={`text-xs px-2 py-0.5 rounded-full ${better ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-700"}`}>
                                                    ...
                                                    <div className="mt-2">
                                                        <div className="text-[11px] text-gray-500">{attrComparison.label}: {Math.round(peer)}</div>
                                                        <div className="w-full bg-gray-200 rounded h-2 overflow-hidden">
 ...
 **********************************/
function StatCompare({ label, me, team, pct = false, fixed2 = false }: { label: string; me: number; team: number; pct?: boolean; fixed2?: boolean }) {
    const m = Number(me || 0);
    ...
    return (
        <div className="rounded-xl border p-3">
            <div className="flex items-center justify-between text-sm">
                <span className="font-medium text-gray-800">{label}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${isBetter ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-700"}`}>
                    {isBetter ? "↑" : "→"}
                </span>
            </div>
            <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                <div>
                    ...
                    <div className="font-semibold">
                        {pct ? fmtPct(m) : fixed2 ? m.toFixed(2) : fmtNum(m)}
                    </div>
                </div>
                <div>
                    <div className="text-[11px] text-gray-500">Time (média)</div>
                    <div className="font-semibold">
                        {pct ? fmtPct(t) : fixed2 ? t.toFixed(2) : fmtNum(t)}
                    </div>
                </div>
            </div>
        </div>
