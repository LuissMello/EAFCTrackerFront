...
export default function PlayerStatisticsByPlayerPage() {
      ...
      try {
        ...
        const blocks: DayBlock[] = (Array.isArray(data) ? data : [])
          .map((row) => {
            ...
            const clubsArr = Array.isArray(row?.statistics?.clubs) ? row.statistics!.clubs! : [];

            const gf = clubsArr.reduce((acc, c: any) => acc + toNum(c?.goalsFor ?? c?.GoalsFor), 0);
            const ga = clubsArr.reduce((acc, c: any) => acc + toNum(c?.goalsAgainst ?? c?.GoalsAgainst), 0);
            ...
            return {
              ...
              goalsFor: gf,
              goalsAgainst: ga,
              players: Array.isArray(row?.statistics?.players) ? (row.statistics!.players as PlayerStats[]) : [],
            };
          })
  ...
  const playerOptions: SimplePlayerOption[] = useMemo(() => {
    ...
    for (const d of days) {
      for (const p of d.players) {
        ...
        if (!pid) continue;
        const name =
          (p as any).playerName ?? (p as any).PlayerName ?? (p as any).proName ?? (p as any).ProName ?? `Player ${pid}`;
        if (!map.has(pid)) {
          map.set(pid, name);
        }
      }
  ...
  const selectedPlayerId = selectedPlayerIds[0] ?? null; // for backward compatibility
  ...
  // ===== Per day data for all selected players =====
  const perDayByPlayer = useMemo(() => {
    const result: Record<number, PlayerDaySummary[]> = {};
    ...
    for (const pid of selectedPlayerIds) {
      ...
      for (const d of days) {
        const player = d.players.find((p) => {
          const playerId = toNum((p as any).playerId ?? (p as any).PlayerId);
          return playerId === pid;
        });
        if (!player) continue;
      ...
      result[pid] = list.sort((a, b) => b.date.localeCompare(a.date));
    }

    return result;
  }, [days, selectedPlayerIds]);

  // ===== Chart data for selected players =====
  const chartDataForPlayer = useMemo(() => {
    if (selectedPlayerIds.length === 0) return null;

    // Get all unique dates across all selected players
    const allDates = new Set<string>();
    for (const pid of selectedPlayerIds) {
      const data = perDayByPlayer[pid] ?? [];
      for (const d of data) {
        allDates.add(d.date.slice(0, 10));
      }
    }
    ...
    const createDatasetsForMetric = (
      ...
      selectedPlayerIds.forEach((pid, idx) => {
        ...
        for (const d of playerData) {
          dataByDate.set(d.date.slice(0, 10), d);
        }

        const values = sortedDates.map((date) => {
          const dayData = dataByDate.get(date);
          return dayData ? getValue(dayData) : null;
        });

        const label = selectedPlayerIds.length > 1 ? `${playerName}` : metricName;
        ...
        datasets.push({
          ...
          pointHoverRadius: 5,
          fill: selectedPlayerIds.length === 1,
          spanGaps: true,
        });

        // Add trendline only for single player
        if (selectedPlayerIds.length === 1) {
          const validValues = values.map(v => v ?? 0);
          datasets.push({
            label: `${metricName} (tendência)`,
            data: movingAvg(validValues, 3),
  ...
  const summary: SummaryBox | null = useMemo(() => {
    ...
    const baseItems = selectedDayKey === "all" ? allGames : matchesForSelectedDay;
    ...
    const daysCount = selectedDayKey === "all" ? perDayForPlayer.length : 1;
    const matchesPerDay = daysCount > 0 ? totalMatches / daysCount : totalMatches;

    const scopeLabel =
      selectedDayKey === "all" ? `${fmtBRFromISO(dateFrom)} — ${fmtBRFromISO(dateTo)}` : fmtBRFromISO(selectedDayKey);
  ...
  }, [selectedPlayerId, selectedDayKey, allGames, matchesForSelectedDay, perDayForPlayer.length, dateFrom, dateTo]);
  ...
  const gamesForRanking: GameRow[] = useMemo(() => {
    const base = selectedDayKey === "all" ? allGames : matchesForSelectedDay;
  ...
  const bestGoalsGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => b.goals - a.goals || b.rating - a.rating).slice(0, 5);
  }, [gamesForRanking]);

  const worstGoalsGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => a.goals - b.goals || a.rating - b.rating).slice(0, 5);
  }, [gamesForRanking]);

  const bestAssistsGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => b.assists - a.assists || b.rating - a.rating).slice(0, 5);
  }, [gamesForRanking]);
  ...
  const bestTacklesGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => b.tacklesMade - a.tacklesMade || b.tacklePct - a.tacklePct).slice(0, 5);
  }, [gamesForRanking]);

  const worstTacklesGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => a.tacklePct - b.tacklePct || a.tacklesMade - b.tacklesMade).slice(0, 5);
  }, [gamesForRanking]);

  const bestPassesGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => b.passPct - a.passPct || b.passesMade - a.passesMade).slice(0, 5);
  }, [gamesForRanking]);

  const worstPassesGames = useMemo(() => {
    return [...gamesForRanking].sort((a, b) => a.passPct - b.passPct || a.passesMade - b.passesMade).slice(0, 5);
  }, [gamesForRanking]);

  // ===== RANKINGS POR DIA (quando "Todos" está selecionado) =====
  const bestGoalsDays = useMemo(() => {
    return [...dayRowsForRanking].sort((a, b) => b.goals - a.goals || b.rating - a.rating).slice(0, 5);
  }, [dayRowsForRanking]);

  const worstGoalsDays = useMemo(() => {
    return [...dayRowsForRanking].sort((a, b) => a.goals - b.goals || a.rating - b.rating).slice(0, 5);
  }, [dayRowsForRanking]);

  const bestAssistsDays = useMemo(() => {
    return [...dayRowsForRanking].sort((a, b) => b.assists - a.assists || b.rating - a.rating).slice(0, 5);
  }, [dayRowsForRanking]);
  ...
  const bestPassesDays = useMemo(() => {
    return [...dayRowsForRanking].sort((a, b) => b.passPct - a.passPct || b.passesMade - a.passesMade).slice(0, 5);
  }, [dayRowsForRanking]);

  const worstPassesDays = useMemo(() => {
    return [...dayRowsForRanking].sort((a, b) => a.passPct - b.passPct || a.passesMade - b.passesMade).slice(0, 5);
  }, [dayRowsForRanking]);
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-gray-500">
                {selectedPlayerIds.length === 0
                  ...
                  ? `Selecionado: ${selectedPlayerName} (clique em outro para comparar)`
            ...
            {playerOptions.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {playerOptions.map((p) => {
                  const isActive = selectedPlayerIds.includes(p.playerId);
                  const selectionIndex = selectedPlayerIds.indexOf(p.playerId);
                  return (
                    <button
                      key={p.playerId}
                      type="button"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
            <div className="flex items-center gap-2">
              <h2 className="text-sm sm:text-base font-semibold">
                Resumo geral — {selectedDayKey === "all" ? "período selecionado" : "dia selecionado"}
              </h2>
      ...
      {selectedPlayerId && perDayForPlayer.length > 0 && (
        <section className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <h2 className="text-sm sm:text-base font-semibold">Melhor dia do jogador e rankings</h2>
              <span className={modeBadgeClass}>{modeLabel}</span>
            </div>
          </div>
            ...
            <div className="rounded-lg border bg-white p-3 flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-4 text-xs sm:text-sm text-gray-700 mt-1">
                ...
                <span>
                  ...
                  {bestDay.passPct.toFixed(1)}%)
                </span>
                <span>
                  Desarmes: <strong>{bestDay.tacklesMade}</strong> / {bestDay.tacklesAttempted} (
                  {bestDay.tacklePct.toFixed(1)}%)
                </span>
                <span>
                  Nota média: <strong>{bestDay.rating.toFixed(2)}</strong>
                </span>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-semibold">
                  {selectedPlayerName} — jogos no dia <DateBadge dateISO={selectedDayKey} colorMap={dateColorMap} />
                </h2>
                <span className={modeBadgeClass}>{modeLabel}</span>
              </div>
          ...
          {!matchesLoading && !matchesError && gameHighlights.length > 0 && bestGameOfDay && (
            <div className="rounded-lg border bg-white p-3 flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-4 text-xs sm:text-sm text-gray-700 mt-1">
                ...
                <span>
                  Participações: <strong>{bestGameOfDay.participations}</strong>
                </span>
                <span>
                  Passes: <strong>{bestGameOfDay.passesMade}</strong> / {bestGameOfDay.passesAttempted} (
                  {bestGameOfDay.passPct.toFixed(1)}%)
                </span>
                <span>
                  Desarmes: <strong>{bestGameOfDay.tacklesMade}</strong> / {bestGameOfDay.tacklesAttempted} (
                  {bestGameOfDay.tacklePct.toFixed(1)}%)
                </span>
                <span>
                  Defesas: <strong>{bestGameOfDay.saves}</strong>
                </span>
                <span>
                  Nota: <strong>{bestGameOfDay.rating.toFixed(2)}</strong>
                </span>
              </div>
            </div>
          )}
