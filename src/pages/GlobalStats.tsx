...
export default function PlayerStatisticsPage() {
  ...
  // Calculando percentual de passes e desarmes
  const calcPassAccuracy = (made: number, attempts: number) => {
    return attempts > 0 ? (made / attempts) * 100 : 0;
  };

  const calcTackleSuccess = (made: number, attempts: number) => {
    return attempts > 0 ? (made / attempts) * 100 : 0;
  };
              ...
              <tbody>
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    ...
                    {calcPassAccuracy(clubStats.totalPassesMade, clubStats.totalPassAttempts).toFixed(1)}%)
                  </td>
                  <td className="px-3 py-2">
                    {clubStats.totalTacklesMade} / {clubStats.totalTackleAttempts} (
                    {calcTackleSuccess(clubStats.totalTacklesMade, clubStats.totalTackleAttempts).toFixed(1)}%)
                  </td>
                  <td className="px-3 py-2">
            ...
            {sortedPlayers.map(player => (
              <tr key={player.playerId} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2">
                  ...
                  {calcPassAccuracy(player.totalPassesMade, player.totalPassAttempts).toFixed(1)}%)
                </td>
                <td className="px-3 py-2">
                  {player.totalTacklesMade} / {player.totalTackleAttempts} (
                  {calcTackleSuccess(player.totalTacklesMade, player.totalTackleAttempts).toFixed(1)}%)
                </td>
